...
impl TrackService {
    ...
    pub fn resize_clip(
        ...
        let Some((track, clip)) = self.resize_clip_skip_overlap_check(id, start, end, pos, tx)
        else {
            return None;
        };
        // Fix overlaps
        ...
        Some((clip, track.clone(), added_clips, deleted_clips))
    }

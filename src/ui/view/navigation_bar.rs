use egui::{Margin, Rect, Sense, Spacing, Ui, UiBuilder, Vec2, pos2, vec2};
use egui_phosphor::{
    fill::{ARROW_LINE_UP, ARROWS_IN_LINE_VERTICAL, LINE_SEGMENTS},
    regular::ARROWS_IN_SIMPLE,
};
...
impl UINavigationBar {
    ...
    pub fn ui(&mut self, ui: &mut Ui, state: &mut ToniqueProjectState, track_width: f32) {
        ...
        ui.horizontal(|ui| {
                ...
                && let Some(mouse_pos) = nav_bar_response.hover_pos()
            {
                ...
                state.grid.zoom_around(factor, mouse_pos.x, nav_bar_rect);
            }
    ...
    fn right_ui(&mut self, ui: &mut Ui, state: &mut ToniqueProjectState) {
        ui.scope(|ui| {
            ...
            if ui
                .add(SquareButton::new(ARROWS_IN_SIMPLE).tooltip("Close All"))
                .clicked()
            {
                state.set_all_close(true);
            };
            ui.add(SquareButton::new(LINE_SEGMENTS).tooltip("Automate"));
        });
    }
}

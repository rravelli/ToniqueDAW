...
use crate::{
    ...
    components::{
        buttons::left_aligned_selectable, left_panel::DragPayload, waveform::UIWaveform,
...
impl FileBrowser {
    ...
    pub fn ui(&mut self, ui: &mut Ui, tx: &mut Producer<GuiToPlayerMsg>) {
        ...
        if self.preview_state == PlaybackState::Playing {
            ...
            if self.selected_audio.is_none() {
                let _ = tx.push(GuiToPlayerMsg::PausePreview());
    ...
    fn preview_window(&mut self, ui: &mut Ui, tx: &mut Producer<GuiToPlayerMsg>) {
        Frame::new()
